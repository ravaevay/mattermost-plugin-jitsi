...
- Use a `/jitsi persistent [topic]` command to create a persistent meeting link that never expires. Each click on "Join Meeting" generates a fresh JWT on the fly, so the link can be pinned in a channel and reused indefinitely.
...
- JWT tokens include `channel_id` in the context, allowing Jibri to determine which channel a meeting was started from without querying the Mattermost API.
...
4. (Optional) If your Jitsi server uses JSON Web Tokens (JWT) for authentication, set:
  ...
  - When JWT is enabled, the `channel_id` of the Mattermost channel is included in the JWT context, which can be used by Jibri or other integrations.
...
| Command | Description |
|---------|-------------|
| `/jitsi` | Start a new meeting in the current channel |
| `/jitsi start [topic]` | Start a new meeting with a specified topic |
| `/jitsi persistent [topic]` | Create a persistent meeting link that never expires |
| `/jitsi settings see` | View your current user settings |
| `/jitsi settings [setting] [value]` | Update your user settings |
| `/jitsi help` | Show help text |
